class UsersController < ApplicationController
  before_filter :check_for_admin, only: [:new, :index, :edit, :destroy, 
    :transfer]
  ...
  def destroy
    ...
    @user = User.find(params[:id])
    if @user.admin ==  true
      flash[:error] = 'Since you are an admin so you can not delete your self so 
                      first you need to transfer your adminstration then ask admin to delete yourself' 
    else
      @user.destroy  
